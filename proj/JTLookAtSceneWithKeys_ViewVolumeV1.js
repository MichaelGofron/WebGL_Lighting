...
//  
  ...
  '  float nDotH = max(dot(H, normalize(normal)), 0.0); \n' +
      // (use max() to discard any negatives from lights below the surface)
...
//     || !u_Ks || !u_Kshiny
  ...
  if (!u_MvpMatrix) { 
    console.log('Failed to get u_MvpMatrix or u_ProjMatrix');
    return;
  }
  ...
  */
  ...
  var tick = function(){
    ...
    draw(gl, u_MvpMatrix, mvpMatrix);   // Draw the triangles
    requestAnimationFrame(tick, canvas);   // Request that the browser ?calls tick
  };
  tick();
  winResize(gl,u_MvpMatrix,mvpMatrix);
...
function draw(gl, u_MvpMatrix, mvpMatrix) {
//==============================================================================
  ...
  // Draw the scene:
  drawMyScene(gl, u_MvpMatrix, mvpMatrix);
}
...
function drawMyScene(myGL, myu_MvpMatrix, myMvpMatrix) {
...
// Rotate to make a new set of 'world' drawing axes: 
 ...
 // myMvpMatrix
  myMvpMatrix.rotate(-90.0, 1,0,0); // new one has "+z points upwards",
                                      // made by rotating -90 deg on +x-axis.
                                      // Move those new drawing axes to the 
                                      // bottom of the trees:
  myMvpMatrix.translate(0.0, 0.0, -0.6);  
  myMvpMatrix.scale(0.4, 0.4,0.4);    // shrink the drawing axes 
  ...
  myMvpMatrix.translate(0,0,2.5);

  pushMatrix(myMvpMatrix); // full body
  myMvpMatrix.scale(1.5,1.5,1.5);
  myMvpMatrix.rotate(currentAngle,0,0,1);
  myGL.uniformMatrix4fv(myu_MvpMatrix,false,myMvpMatrix.elements);
  myGL.drawArrays(myGL.TRIANGLES,
                robotBodyStart/floatsPerVertex,
                cubeVertsToDraw/floatsPerVertex);

  pushMatrix(myMvpMatrix); // right leg
  myMvpMatrix.translate(0.4,0,-1.2);
  myMvpMatrix.scale(0.3,0.6,0.5);
  myGL.uniformMatrix4fv(myu_MvpMatrix,false,myMvpMatrix.elements);
  myGL.drawArrays(myGL.TRIANGLES,
                  robotRightLegStart/floatsPerVertex,
                  cubeVertsToDraw/floatsPerVertex);
  myMvpMatrix = popMatrix(); // right leg

  pushMatrix(myMvpMatrix); // left leg
  myMvpMatrix.translate(-0.4,0,-1.2);
  myMvpMatrix.scale(0.3,0.6,0.5);
  myGL.uniformMatrix4fv(myu_MvpMatrix,false,myMvpMatrix.elements);
  myGL.drawArrays(myGL.TRIANGLES,
                  ...
                  cubeVertsToDraw/floatsPerVertex);
  myMvpMatrix = popMatrix();

  pushMatrix(myMvpMatrix); // left arm
  myMvpMatrix.translate(-1.4,0,0);
  myMvpMatrix.scale(0.4,0.7,0.7);
  myGL.uniformMatrix4fv(myu_MvpMatrix,false,myMvpMatrix.elements);
  myGL.drawArrays(myGL.TRIANGLES,
  ...
  pushMatrix(myMvpMatrix); // right arm shoulder
  myMvpMatrix.translate(1.2,0,0.4);
  myMvpMatrix.scale(0.2,0.2,0.2);
  myMvpMatrix.rotate(currentAngle,0,1,0);
  myGL.uniformMatrix4fv(myu_MvpMatrix,false,myMvpMatrix.elements);
  myGL.drawArrays(myGL.TRIANGLES,
                 robotRightShoulderStart/floatsPerVertex,
                 cubeVertsToDraw/floatsPerVertex);

  pushMatrix(myMvpMatrix); // right arm 
  myMvpMatrix.translate(3.0,0,-1.4);
  myMvpMatrix.scale(2.0,2.6,2.6);
  myMvpMatrix.rotate(currentAngle,0,1,0);
  myGL.uniformMatrix4fv(myu_MvpMatrix,false,myMvpMatrix.elements);
  myGL.drawArrays(myGL.TRIANGLES,
  ...
  myMvpMatrix = popMatrix(); // right arm 
  myMvpMatrix = popMatrix(); // right arm shoulder

  myMvpMatrix = popMatrix(); // full body


  myMvpMatrix.translate(10.0,0,0.0);
  ...
  // Make test sphere
  pushMatrix(myViewMatrix); // sphere
  myViewMatrix.translate(1,-6,0);
...
function winResize(gl,u_MvpMatrix,mvpMatrix) {
//==============================================================================
...
// contains:  <body onload="main()" onresize="winResize()">
  ...
  var u_MvpMatrix = nuGL.getUniformLocation(nuGL.program, 'u_MvpMatrix');
  var mvpMatrix = new Matrix4();


  draw(nuGL,u_MvpMatrix,mvpMatrix);
}
